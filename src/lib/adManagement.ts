...
export type AdActionType = 'kill' | 'remove' | 'pause' | 'activate' | 'duplicate' | 'optimize' | 'scale';
...
export async function triggerAdAction(
  ...
  try {
    ...
    if (actionType === 'kill') newStatus = 'deleted';
    else if (actionType === 'pause') newStatus = 'paused';
    else if (actionType === 'activate') newStatus = 'active';
