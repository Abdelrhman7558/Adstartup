...
export async function triggerEntityActionWebhook(
  userId: string,
  adName: string,
  userEmail: string,
  actionName: string,
  ...
  try {
    ...
    const webhookId = await sendWebhookWithRetry(userId, actionType, payload);
    return webhookId;
  } catch (err) {
    console.error(`[Webhooks] Error triggering ${actionType} webhook:`, err);
    throw err;
  }
}
  userId: string,
