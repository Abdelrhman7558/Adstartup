...
export default function MetaConnectionStatus({ userId, onStatusChange, className = '' }: MetaConnectionStatusProps) {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      const { data, error: invokeError } = await supabase.functions.invoke('disconnect-meta', {
        body: { userId }
      });
      ...
      // Check response body for errors (Edge Function returns 200 with error in body)
