...
export default function AssetsPanel({ userId }: AssetsPanelProps) {
  ...
  const handleAddAsset = async (e: React.FormEvent) => {
    ...
    if (!workspaceId) {
      toast('Workspace not initialized', 'error');
      return;
    }

    if (assetType !== 'image' && assetType !== 'video' && !assetText) {
      toast('Please enter asset content', 'error');
      return;
    }
    ...
    try {
      ...
      toast(`Failed to create asset: ${error.message}`, 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteAsset = async (assetId: string) => {
    ...
    try {
      ...
      toast(`Failed to delete asset: ${error.message}`, 'error');
    }
  };
