...
import { ChevronUp, ChevronDown, Trash2, Pause, Play, Copy, Zap, TrendingUp } from 'lucide-react';
...
interface AdsManagementTableProps {
  ...
  onAction: (adId: string, action: 'kill' | 'remove' | 'pause' | 'activate' | 'duplicate' | 'optimize' | 'scale') => Promise<void>;
...
export default function AdsManagementTable({
  ...
  const handleAction = async (adId: string, action: 'kill' | 'remove' | 'pause' | 'activate' | 'duplicate' | 'optimize' | 'scale') => {
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {/* New Bulk/Single Actions */}
                      <button
                        onClick={() => handleAction(ad.id, 'duplicate')}
                        disabled={isProcessing}
                        className="rounded p-1 text-gray-400 transition hover:bg-gray-700 hover:text-blue-400 disabled:opacity-50"
                        title="Duplicate ad"
                      >
                        <Copy size={16} />
                      </button>
                      <button
                        onClick={() => handleAction(ad.id, 'optimize')}
                        disabled={isProcessing}
                        className="rounded p-1 text-gray-400 transition hover:bg-gray-700 hover:text-purple-400 disabled:opacity-50"
                        title="Optimize ad"
                      >
                        <Zap size={16} />
                      </button>
                      <button
                        onClick={() => handleAction(ad.id, 'scale')}
                        disabled={isProcessing}
                        className="rounded p-1 text-gray-400 transition hover:bg-gray-700 hover:text-emerald-400 disabled:opacity-50"
                        title="Scale ad"
                      >
                        <TrendingUp size={16} />
                      </button>
