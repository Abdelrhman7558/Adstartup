...
import { Trash2, AlertTriangle, Loader, PlayCircle, PauseCircle, CheckCircle2, Copy, Zap, TrendingUp } from 'lucide-react';
  ...
  const handleKillAllAds = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast('All ads have been killed successfully', 'success');
      } else {
      ...
      toast('Failed to kill all ads. Please try again.', 'error');
    } finally {
      setKillingAll(false);
    }
  };
  ...
  const handleRemoveAd = async (ad: Ad) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast(`Ad "${ad.name}" has been removed successfully`, 'success');
      } else {
        throw new Error('Webhook failed');
      }
    } catch (error) {
      ...
      toast('Failed to remove ad. Please try again.', 'error');
    } finally {
      setRemovingAdId(null);
    }
  };

  const handleEntityAction = async (ad: Ad, actionName: string, webhookEndpoint: string, successMessage: string) => {
    setProcessingAdId(ad.id);
    try {
      const validatedUserId = validateUserId(userId);

      const response = await fetch(`https://n8n.srv1181726.hstgr.cloud/webhook-test/${webhookEndpoint}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        await fetchAds();
        logWebhookCall('POST', webhookEndpoint, validatedUserId, true);
      ...
      if (error instanceof WebhookValidationError) {
        console.error(`[handle${actionName}] Validation error:`, error.message);
      } else {
        console.error(`Error with ${actionName}:`, error);
      }
      logWebhookCall('POST', webhookEndpoint, userId, false, { error: String(error) });
      toast(`Failed to ${actionName.toLowerCase()} ad. Please try again.`, 'error');
    } finally {
      setProcessingAdId(null);
    }
  };

  const handleDuplicateAd = (ad: Ad) => handleEntityAction(ad, 'Duplicate', 'Duplicate', `Ad "${ad.name}" duplicated successfully`);
  const handleOptimizeAd = (ad: Ad) => handleEntityAction(ad, 'Optimize', 'Optimize', `Optimization applied to "${ad.name}"`);
  const handleScaleAd = (ad: Ad) => handleEntityAction(ad, 'Scale', 'Scale', `Scaling applied to "${ad.name}"`);
                  ...
                  <tr key={ad.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      ...
                      <button
                        ...
                        title="Duplicate"
                        className="ml-2 inline-flex items-center gap-2 px-3 py-1.5 bg-blue-100 hover:bg-blue-200 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-700 dark:text-blue-400 font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                      >
                        <Copy className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleOptimizeAd(ad)}
                        disabled={processingAdId === ad.id}
                        title="Optimize"
                        className="ml-2 inline-flex items-center gap-2 px-3 py-1.5 bg-purple-100 hover:bg-purple-200 dark:bg-purple-900/30 dark:hover:bg-purple-900/50 text-purple-700 dark:text-purple-400 font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                      >
                        <Zap className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleScaleAd(ad)}
                        disabled={processingAdId === ad.id}
                        title="Scale"
                        className="ml-2 inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-100 hover:bg-emerald-200 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400 font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                      >
